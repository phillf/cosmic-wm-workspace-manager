...
assistant/control workspace and remains outside managed cold-start and live
reroute operations.
...
controlled login or when intended profile-defined applications, including
managed browser windows, need to be launched.
...
Selecting `sysadmin` in that chooser invokes
`~/.local/bin/start-sysadmin-cold`.
